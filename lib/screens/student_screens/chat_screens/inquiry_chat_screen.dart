...
class InquiryChatScreen extends StatefulWidget {
  ...
  InquiryChatScreen(this.data, {Key? key}) : super(key: key);
...
class _InquiryChatScreenState extends State<InquiryChatScreen> {
  ...
  Future getSpecificInquiryChat() async {
    ...
    try {
      final response = await http.get(
        Uri.parse(
            "${AppUrl.baseUrl}messages_data?auth_token=$token&student_id=$studentId&type=inquiry&inquiry_thread_code=${widget.data[1] ?? widget.data['inquiry_thread_code']}"),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(
          "${widget.data[0] ?? widget.data['reciever_data']['reciever_name']}",
                                                          ...
                                                          onTap: () {
                                                            Io.Platform.isWindows
                                                                ? Navigator.push(
                                                                    context,
                                                                    MaterialPageRoute(
                                                                        builder: (context) => WebviewDsiplayDesktop(Uri.decodeFull(snapshot.data['messages'][0]['history'][index]
                                                                            [
                                                                            'attached_file_name']))))
                                                                : Navigator.push(
                                                                    context,
                                                                    MaterialPageRoute(
                                                                        builder:
                                                                            (context) =>
                                                                                WebviewDisplayMacos(Uri.encodeFull(widget.data['history'][index]['attached_file_name']))));
                                                          },
                                                          child: Container(
                                                            height: 100.h,
                                                            width: 50.w,
